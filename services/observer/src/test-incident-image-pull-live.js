...
try {
  const result = await db.query(
    `
      ...
      WHERE
        cluster_id = $1
        AND kind = 'Pod'
        AND name LIKE 'kubedoctor-image-pull-test%'
  ...
  if (result.rows.length === 0) {
    throw new Error("ImagePull test Pod not found in resource_snapshots");
  }

  const snapshot = result.rows[0];

  console.log("[TEST] Found ImagePull snapshot:", snapshot.name, snapshot.uid);
