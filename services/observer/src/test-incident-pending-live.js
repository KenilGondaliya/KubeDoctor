...
try {
  const result = await db.query(
    ...
    WHERE
      cluster_id = $1
      AND kind = 'Pod'
      AND name = 'kubedoctor-pending-test'
    LIMIT 1
    `,
    [CLUSTER_ID],
  );
  ...
  if (result.rows.length === 0) {
    throw new Error("Pending test Pod not found in resource_snapshots");
  }

  const snapshot = result.rows[0];

  console.log("[TEST] Found Pending snapshot:", snapshot.name, snapshot.uid);
