...
try {
  const result = await db.query(
    `
      ...
      WHERE
        cluster_id = $1
        AND kind = 'Pod'
        AND name = 'kubedoctor-readiness-test-5dbbb675f8-w5kkp'
      LIMIT 1
      `,
    [CLUSTER_ID],
  );
  ...
  if (result.rows.length === 0) {
    throw new Error("Readiness test Pod not found in resource_snapshots");
  }

  const snapshot = result.rows[0];

  console.log("[TEST] Found readiness snapshot:", snapshot.name, snapshot.uid);
  ...
  console.log("[TEST] Processing real snapshot...");
